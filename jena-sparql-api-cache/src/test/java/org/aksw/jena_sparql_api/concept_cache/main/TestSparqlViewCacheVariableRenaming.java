...
public class TestSparqlViewCacheVariableRenaming {
    ...
    public void testSparqlViewCacheRenaming() throws IOException {
        // Reader reader = new InputStreamRnew FileInputStream(file) //new
        ...
        // InputStreamReader(r.getInputStream(), StandardCharsets.UTF_8));
        // jsonReader.setLenient(true);
        //
        // Type listType = new TypeToken<ArrayList<String>>() {}.getType();
        // List<String> queryStrs = gson.fromJson(jsonReader, listType);

        Model model = SparqlQcReader
                .readResources("sparqlqc/1.4/benchmark/noprojection/*");
        QueryExecutionFactory qef = FluentQueryExecutionFactory.from(model)
                .create();
        ResultSet rs = qef
                .createQueryExecution(
                        "SELECT ?c { ?s <http://ex.org/ontology/content> ?c }")
                .execSelect();

        List<String> queryStrs = new ArrayList<>();
        rs.forEachRemaining(
                b -> queryStrs.add(b.get("c").asLiteral().getString()));

        model.write(System.out, "TURTLE");

        SparqlQueryParser sparqlParser = SparqlQueryParserImpl
                .create(Syntax.syntaxARQ);
        List<Query> queries = queryStrs.stream().map(sparqlParser)
    ...
    public static void testVariableRenaming(Query baseQuery)
            throws IOException {

        Collection<Var> vars = PatternVars.vars(baseQuery.getQueryPattern());

        QuadFilterPatternCanonical qfpc = SparqlCacheUtils
                .transform2(baseQuery);
        List<Var> baseVars = baseQuery.getProjectVars();

        Generator<Var> gen = new VarGeneratorBlacklist(
                VarGeneratorImpl2.create("v"), vars);

        Map<Var, Node> varMap = vars.stream()
                .collect(Collectors.toMap(v -> v, v -> (Node) gen.next()));

        System.out.println("Trying to rename: " + baseQuery + " with varmap " + varMap);



        Query renamedQuery = QueryTransformOps.transform(baseQuery, varMap);
        // List<Var> renamedVars = renamedQuery.getProjectVars();
        QuadFilterPatternCanonical renamedQfpc = SparqlCacheUtils
                .transform2(renamedQuery);

        SparqlViewCache sparqlViewCache = new SparqlViewCacheImpl();

        sparqlViewCache.index(qfpc,
                new TableData(baseVars, Collections.emptyList()));
        CacheResult cr = sparqlViewCache.lookup(renamedQfpc);

        System.out.println("Cache lookup - got " + cr.getTables().size()
                + " candidate tables");

        // SparqlViewCache.

        // SparqlV
        // public CacheResult lookup(QuadFilterPatternCanonical queryQfpc) {
